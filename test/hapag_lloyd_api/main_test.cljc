(ns hapag_lloyd_api.main-test
  "Contract + behavioral test for the hapag_lloyd_api-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "hapag_lloyd_api-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'hapag_lloyd_api.main-test)]
