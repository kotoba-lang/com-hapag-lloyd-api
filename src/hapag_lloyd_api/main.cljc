(ns hapag_lloyd_api.main
  "Kotodama WASM entrypoint for the Hapag Lloyd API clean-room actor (L4) — Clojure port.
  ...
  facts (`hapag_lloyd_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hapag_lloyd_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"  :plural "tradeitems"  :id-prefix "hapagllo_tra"
    :fields [:gtin :description :brand]         :required [:gtin :description]
    :coerce {}                        :refs {}}
   {:entity "Event"      :plural "events"       :id-prefix "hapagllo_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                        :refs {}}
   {:entity "Document"   :plural "documents"    :id-prefix "hapagllo_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                        :refs {}}
   {:entity "Container"  :plural "containers"   :id-prefix "hapagllo_con"
    :fields [:bicCode :type :status]            :required [:bicCode :type]
    :coerce {}                        :refs {}}
   {:entity "Party"      :plural "parties"      :id-prefix "hapagllo_par"
    :fields [:gln :name :role]                  :required [:gln :name]
    :coerce {}                        :refs {}}
   {:entity "Shipment"   :plural "shipments"    :id-prefix "hapagllo_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"hapag_lloyd_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hapag_lloyd_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hapag_lloyd_api-compat/ready)
